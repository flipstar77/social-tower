/**
 * PerformanceCharts - Manages dashboard performance charts using Apache ECharts
...
class PerformanceCharts {
    ...
    /**
     * Validate that a container exists and has dimensions
     */
    validateContainer(containerId) {
        ...
        if (width === 0 || height === 0) {
            console.error(`❌ Container #${containerId} has zero dimensions: ${width}x${height}px`);
            console.log('Container computed style:', window.getComputedStyle(container).height);
            return null;
        }

        console.log(`✅ Container #${containerId}: ${width}x${height}px`);
        return container;
    }
    ...
    initVerticalBarChart() {
        const container = this.validateContainer('chart-vertical-bars');
        if (!container) return;
     ...
     */
    initHorizontalBars() {
        const container = this.validateContainer('chart-horizontal-bars');
        if (!container) return;
