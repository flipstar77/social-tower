...
class TowerAnalytics {
    ...
    renderStatsCards(stats, totals, rates) {
        ...
        // Apply preset filtering first, then filter out hidden cards and category toggles
        ...
        const visibleCards = presetCards.filter((card, index) => {
            ...
            if (this.hiddenCards && this.hiddenCards.has(originalIndex)) {
                return false;
            }

            // Check if card's category is toggled off
            const category = this.getCategoryForLabel(card.label);
            if (!this.categoryToggles[category]) {
                return false;
            }
